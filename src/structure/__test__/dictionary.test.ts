...
import { murmur2_32, sum32 } from '../../algorithm/hash';
...
describe('dictionary', () => {
    ...
    describe('hash collisions', () => {
        ...
        beforeEach(() => {
            ...
            dict = new Dictionary<string>(list, {
                size: 2,
                hashFn: sum32,
            });
        });
        it('should get value', () => {
            expect(dict.get('keyA')).toBe('A');
            expect(dict.get('kAye')).toBe('B');
            expect(dict.get('kyeA')).toBe('C');
        })
    });
});
